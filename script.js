const gradePoints = { A: 5, B: 4, C: 3, D: 2, E: 1, F: 0 };
    ...
    function exportToPDF() {
      ...
      html2pdf()
        .set({ margin: 0.5, filename: 'GPA-CGPA-Report.pdf', image: { type: 'jpeg', quality: 0.98 }, html2canvas: { scale: 2 } })
        .from(element)
        .save();
    }
    ...
    function updateChart() {
      ...
      for (let i = 1; i <= semesterCount; i++) {
        ...
        courses.forEach(c => {
          const grade = c.children[1].value;
          const credit = parseFloat(c.children[2].value);
          if (grade && !isNaN(credit)) {
            totalPoints += gradePoints[grade] * credit;
            totalCredits += credit;
          }
        ...
        if (totalCredits > 0) {
          const gpa = totalPoints / totalCredits;
          const label = document.querySelector(`#course-list-${i}`).closest('.semester').querySelector('.semester-title').value;
          labels.push(label);
          data.push(gpa.toFixed(2));
        }
      }
      ...
      chart = new Chart(ctx, {
        ...
        data: {
          labels,
          datasets: [{ label: 'Semester GPA', data, backgroundColor: '#007bff' }]
        },
        options: {
          responsive: true,
